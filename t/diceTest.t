...
ok($output eq "1d10: 4 = \x024\x02", "Parse gives correct output for one die");

$mockRand->mock('rand' => gen_rand(4,8));
($total, $output) = LogiosDice::parse("2d10","sum");
ok($total == 12, "Parse rolls two die correctly");
ok($output eq "2d10: 4 8 = \x0212\x02", "Parse gives correct output for two dice");

#addition and subtraction of constants
$mockRand->mock('rand' => gen_rand(4,8));
($total, $output) = LogiosDice::parse("2d10+2","sum");
ok($total == 14, "Parse adds numbers correctly");
ok($output eq "2d10: 4 8 = \x0212\x02 || +\x022\x02", "Parse gives correct output when adding");

$mockRand->mock('rand' => gen_rand(4,8));
($total, $output) = LogiosDice::parse("2d10-2","sum");
ok($total == 10, "Parse subtracts numbers correctly");
ok($output eq "2d10: 4 8 = \x0212\x02 || -\x022\x02", "Parse gives correct output when subtracting");
